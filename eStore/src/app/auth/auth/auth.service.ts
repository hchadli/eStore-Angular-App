...
export class AuthService{
    ...
    signup(email : string, password : string){
        return this.http.post<AuthResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDlCH4BUrPK970OxilwlwO90_MoTZQlt4E', {
            email: email,
            password: password,
            returnSecureToken : true
        }).pipe(catchError(this.HandleErrorResponse), tap(res => this.HandleAuthentication(res.email, res.localId, res.idToken, +res.expiresIn)));   
            // Tap() operator will of course not change the response body but thus execute some code before giving hand
    }

    login(email:string, password:string){
        return this.http.post<AuthResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDlCH4BUrPK970OxilwlwO90_MoTZQlt4E', {
            email: email,
            password: password,
            returnSecureToken : true
        }).pipe(catchError(this.HandleErrorResponse), tap(res => this.HandleAuthentication(res.email, res.localId, res.idToken, +res.expiresIn)));   
    ...
    private HandleErrorResponse(err : HttpErrorResponse){
        ...
        if(!err.error || !err.error.error) {    // We're checking here if the response error has keys and values for error and error.error
